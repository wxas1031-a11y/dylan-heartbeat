...
function loadScreentime() {
  ...
  try {
    return fs.readJsonSync(SCREENTIME_FILE);
  } catch (e) {
    return {};
  }
}
...
function saveScreentime(data) {
  fs.writeJsonSync(SCREENTIME_FILE, data, { spaces: 2 });
}

function cleanOldRecords(data) {
  const cutoff = Date.now() - 24 * 60 * 60 * 1000;
  for (const app in data) {
    data[app].log = data[app].log.filter(function(e) { return e.t > cutoff; });
    if (data[app].log.length === 0) delete data[app];
  }
}

fastify.get("/api/screentime/toggle/:app", async function(request, reply) {
  const appName = decodeURIComponent(request.params.app);
  const now = Date.now();
...
fastify.get("/api/screentime/query", async function(request, reply) {
  const data = loadScreentime();
  cleanOldRecords(data);
  saveScreentime(data);
  return data;
