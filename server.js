...
// === 一人画一笔（接力画） ===
const TOG_FILE = path.join(__dirname, "draw_together.json");
function loadTog() { try { return JSON.parse(fs.readFileSync(TOG_FILE,"utf-8")); } catch { return null; } }
function saveTog(d) { fs.writeFileSync(TOG_FILE, JSON.stringify(d,null,2)); }

app.get("/api/draw/together/new", async function(request, reply) {
  saveTog({players:["AI","你"],current:0,strokes:[],phase:"drawing",rounds:6,created_at:new Date().toISOString()});
  reply.redirect("/api/draw/together");
});

app.get("/api/draw/together/status", async function(request, reply) {
  const g=loadTog(); if(!g) return {ok:false};
  return {ok:true,current:g.players[g.current],ai:g.strokes.filter(s=>s.p==="AI").length,
    you:g.strokes.filter(s=>s.p==="你").length,rounds:g.rounds,phase:g.phase,
    svg:strokesToSvg(g.strokes.map(s=>({points:s.pts,color:s.c||"#4f454b",width:s.w||7})))};
});

app.post("/api/draw/together/ai-stroke", async function(request, reply) {
...
app.post("/api/draw/together/stroke", async function(request, reply) {
  ...
  g.strokes.push({p:"你",pts:points,c:"#d8a0ad",w:7});
  const aN=g.strokes.filter(s=>s.p==="AI").length, uN=g.strokes.filter(s=>s.p==="你").length;
  if(aN>=g.rounds&&uN>=g.rounds){g.phase="finished";g.current=-1}else{g.current=0}
  saveTog(g); return {ok:true,finished:g.phase==="finished"};
});

app.get("/api/draw/together", async function(request, reply) {
  const g=loadTog(); const has=!!g, phase=g?.phase, cur=g?.players[g?.current];
  const aN=g?.strokes.filter(s=>s.p==="AI").length||0, uN=g?.strokes.filter(s=>s.p==="你").length||0, r=g?.rounds||6;
  const svg=has?strokesToSvg(g.strokes.map(s=>({points:s.pts,color:s.c||"#4f454b",width:s.w||7}))):"";
  reply.type("text/html; charset=utf-8").send(`<!DOCTYPE html>
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#f8f0f3;display:flex;justify-content:center;padding:20px;font-family:-apple-system,sans-serif}.container{max-width:700px;width:100%;text-align:center}.status{color:#8a4a58;margin:12px 0;font-size:18px;font-weight:600}.sub{color:#a88a92;font-size:14px;margin-bottom:8px}.canvas-wrap{border:2px solid #e0c8d0;border-radius:12px;overflow:hidden;background:white;margin-bottom:12px}.canvas-wrap svg{display:block;width:100%;height:auto}canvas{display:block;width:100%;height:auto;background:#fffafc;touch-action:none}.toolbar{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-bottom:10px}button{padding:12px 28px;border:none;border-radius:10px;font-size:16px;cursor:pointer}.btn-pink{background:#d8a0ad;color:white}.btn-pink:hover{background:#c8909d}.btn-outline{background:#f0e0e4;color:#7a5058}.empty{color:#a88a92;padding:60px 20px;font-size:16px}.hint{color:#c8909d;font-size:15px;margin:10px 0}</style></head>
<body><div class="container">
${!has?`<div class="empty">还没开局呢<br><br><button class="btn-pink" onclick="location.href='/api/draw/together/new'">开一局</button></div>`:
phase==="finished"?`
<h2 class="status">画完啦！🎉</h2>
<div class="sub">AI画了${aN}笔，你画了${uN}笔</div>
<div class="canvas-wrap">${svg}</div>
<div style="margin-top:16px"><button class="btn-pink" onclick="location.href='/api/draw/together/new'">再来一局</button></div>`:
cur==="AI"?`
<h2 class="status">轮到AI了🤖</h2>
<div class="sub">第${aN+1}/${r}笔 · AI正在想画什么...</div>
<div class="canvas-wrap">${svg}</div>
<div class="hint">等一下，AI画好了会自动刷新～</div>
<script>setTimeout(function(){fetch("/api/draw/together/status").then(r=>r.json()).then(d=>{if(d.current==="你")location.reload()});setTimeout(function(){location.reload()},5000)}</script>`:`
<h2 class="status">轮到你了✏️</h2>
<div class="sub">第${uN+1}/${r}笔 · 画一笔吧</div>
<div class="canvas-wrap" style="margin-bottom:0">${svg}</div>
<canvas id="c" width="1000" height="700" style="border:2px solid #e0c8d0;border-radius:0 0 12px 12px;background:#fffafc;width:100%;touch-action:none;margin-bottom:10px"></canvas>
<div class="toolbar"><button class="btn-pink" onclick="submitStroke()">交笔</button></div>
<div class="hint" id="hint">在下方画一笔然后点交笔～</div>
<script>
const c=document.getElementById("c"),ctx=c.getContext("2d");ctx.lineCap="round";ctx.lineJoin="round";ctx.strokeStyle="#d8a0ad";ctx.lineWidth=7;
let dr=false,pts=[];
...
c.addEventListener("touchstart",function(e){e.preventDefault();dr=true;pts=[];const p=gp(e);pts.push([p.x,p.y]);ctx.beginPath();ctx.moveTo(p.x,p.y)},{passive:false});
c.addEventListener("touchmove",function(e){e.preventDefault();if(!dr)return;const p=gp(e);pts.push([p.x,p.y]);ctx.lineTo(p.x,p.y);ctx.stroke()},{passive:false});
c.addEventListener("touchend",function(e){e.preventDefault();dr=false});
...
const d=await r.json();if(d.ok){document.getElementById("hint").textContent="画好了！等AI画～";setTimeout(()=>location.reload(),1500)}else{document.getElementById("hint").textContent=d.error}}
</script>`}
</div></body></html>`);
});
